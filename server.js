                    ...
                    var addUpdateNodes = function(_data, highest_shares) {
                        ...
                        query("SELECT * FROM bth_nodes WHERE " + nodeQueryParams.join(" AND "), {}, function(err, rows) {
                            if (!err && rows.length > 0) {
                                ...
                                query("UPDATE bth_nodes SET pou_uptime = pou_shares/"+highest_shares+", pou_bonus=IF(pou_uptime>0.25,IF(pou_uptime<0.5, 5, 10),0), pou_weighed_payout = pou_uptime * pou_payout, pou_sumpayout = pou_weighed_payout + pou_bonus WHERE 1", {}, function() {
                                    query("UPDATE bth_nodes SET "+(_data.blockheight != 0 ? "pou_shares = pou_shares + 1,":"pou_shares = pou_shares,")+" last_reported_on = NOW() WHERE id = '"+entry.id+"'", _data, function() {
